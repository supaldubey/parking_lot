...
public class SlotNumberByColorAction extends AbstractParkingAction {

    public SlotNumberByColorAction(ActionRegistry actionRegistry) {
        super(ActionType.SLOT_LOOKUP_COLOR, actionRegistry);
    }

    @Override
    public String perform(ParkingLot parkingLot, String... arguments) {
        require(arguments != null && arguments.length == 2, "Invalid Arguments for slot by color lookup action. Provided : " + Arrays.toString(arguments));
